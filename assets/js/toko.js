...
let connected = false;
let lastOk = 0;
let scriptName = GetParentResourceName(); // Will need to be refactored.
let clientIp;
let latency = {};
...
async function updateClientIP(endpoint) {
  ...
  if (voipStatus !== OK) {
    ...
    .catch(e => console.error('TokoVOIP: failed to update client IP', e));

    if (res) {
      const ip = await res.text();
      clientIp = ip;
      console.log('TokoVOIP: updated client IP');
      if (websocket && websocket.readyState === websocket.OPEN) websocket.send(
...
async function init(address, serverId) {
  ...
  console.log('TokoVIP: attempt new connection');
  ...
  websocket.onerror = (evt) => {
    console.error('TokoVOIP: ws error - ' + evt.data);
  }
...
function receivedClientCall(evt, data) {
...
ipcRenderer.on("message", receivedClientCall);
